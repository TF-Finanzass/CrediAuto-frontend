...
export class Dashboard {
  ...
  availableCars = computed(() => this.carsStore.cars().filter(c => c.status === CarStatus.Disponible).length);
  reservedCars = computed(() => this.carsStore.cars().filter(c => c.status === CarStatus.Reservado).length);
  soldOutCars = computed(() => this.carsStore.cars().filter(c => c.status === CarStatus.Agotado).length);
  ...
  totalFinanced = computed(() =>
    this.schedulesStore.operations().reduce((sum, op) => sum + op.financedAmount, 0)
  );
  avgTea = computed(() => {
